...
impl GitRepo {
    ...
    pub fn add_worktree(
        ...
        // A branch can survive manual worktree removal; attach to it instead
        // of failing with `-b`.
        let branch_exists = self
            .text(&["branch", "--list", branch])
    ...
    /// Delete a branch that belongs to a removed Minha worktree. Only ever
    /// called with `minha/<run>/<task>` names generated by the harness.
