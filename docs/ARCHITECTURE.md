...
in-process Harness runtime actor
        ...
        |
        +--> local safety-focused clarification preflight
        |       |
        |       +--> clear request: no clarification model call
        |       +--> ordinary greeting/vague conversation: continue without a question
        |       +--> unsafe unknown goal+scope: Luna or deterministic small question batch
        |       +--> persistent high-impact ambiguity: optional Terra advice
        |       +--> user-confirmed structured issue brief
        |
        +--> focused Luna lead or evidence-justified bounded branch plan
        |       |
        |       +--> optional Terra ambiguity consultation
        |       +--> optional Sol high-risk consultation
        |       +--> deterministic task/TODO/health rollup
        |
        ...
        |
        +--> read-only Spark typed completion judge
        |       +--> at most two repair/rejudge cycles for incomplete work
        |
...
### Issue Clarifier state machine

The clarifier is a post-routing state machine shared by the CLI and TUI. Chat and read-only workflows bypass it; actionable mutation routes enter it only when a material detail is unsafe to guess:

```text
local analyze
  -> collecting (0-3 questions in the current batch)
  -> reviewing (rendered brief; no work has started)
  -> confirmed (brief replaces the raw goal for routing)
  -> normal run

collecting/reviewing -> cancelled (durable session; no work)
reviewing -> collecting (edit or keep clarifying)
```

Its internal ambiguity score uses five weighted dimensions: goal 25, reproduction 25, scope 20, constraints 15, and success criteria 15. These local rules, rather than model prose, control entry and exit. The TUI hides this machinery and renders only the current question, choices, and optional note field.

Question generation is bounded to three unresolved dimensions per round and one question per dimension. Luna receives a compact state snapshot and may phrase warmer questions with two or three options. Invalid tags, malformed JSON, duplicate dimensions, resolved dimensions, or unavailable Luna output are discarded in favor of deterministic local questions. The internal score is diagnostic only: automatic clarification requires both unknown goal and unknown scope, so greetings do not produce an ambiguity dashboard. A final brief is confirmed before any edit.

## Office information planes

Minha separates four kinds of information instead of treating one giant chat as the organization:

1. Private agent context contains bounded working state and is never shared truth.
2. Typed DMs and temporary rooms form the communication plane; a message is not canonical merely because it was sent.
3. Tasks, owners, leases, TODOs, phases, and health form structured operational state.
4. Code, tests, evidence, decisions, memories, and content-addressed artifacts form durable knowledge.

Topology follows the task. Small coherent work remains in one focused lane. Independent breadth uses manager/worker scheduling; sequential ownership uses handoffs; evolving artifacts use the board/workspace; three or more mutually dependent owners may use a temporary convergence room. Synchronization occurs at assignments, dependency completion, changed evidence, blockers, gates, and closure—not on every narration token. Accepted consequences from private communication must be promoted into operational state or durable artifacts.

## Provider boundary

The public boundary uses `ProviderId`, `ModelRef`, canonical turn/result types, and the `ProviderClient` trait. ChatGPT Codex translates canonical turns to Responses; DeepSeek V4 translates them to streamed Chat Completions, including fragmented tool-call arguments, thinking/non-thinking controls, reasoning deltas, JSON response mode, finish reasons, keep-alive frames, and cache-hit/miss usage. Dated DeepSeek fallback metadata supplies its context, maximum output, and official per-million-token prices; persisted usage is priced without storing credentials or provider responses. Neither adapter automatically retries model POSTs.
...
Model discovery is a preflight, not a promise. A ChatGPT catalog is reused for 15 minutes, refreshed with an ETag when possible, and may be used as a stale fallback for 24 hours after a refresh failure. DeepSeek uses a versioned capability fallback because its model endpoint does not supply Minha's full capability record. Provider-qualified and legacy bare ChatGPT slugs are accepted. The local quality-per-token router may mix discovered Spark and DeepSeek workers and escalates only after task complexity or failure evidence.
...
| `todo` | Compact per-agent list/revision deltas; advisory and never a prerequisite for verified work |
...
The Issue Clarifier does not receive the full catalog. Luna intake is read-only and sees only `read_files`, `search`, and `github`; Terra intake advice sees no tools. Screenshot paths can be retained as evidence strings but are not interpreted as image content. Clarifier prompts explicitly avoid secret-like paths.
...
- revisioned board entries, typed deduplicated hive messages, versioned rooms/membership, independent consumer cursors, artifacts, and incident records with correlation IDs;
- per-turn input/output/cached/reasoning usage, account rate-limit snapshots, and model catalogs with ETags;
- local cache entries and statistics, indexed book metadata, compaction checkpoints, issue intakes, provider-balance baselines, TODO/memory state, and office-room state.

The pre-production contract is a clean schema v1. On file-backed open, any prototype database whose `user_version` is neither 0 nor 1 is atomically renamed to a timestamped `prototype-vN-*.bak` (including WAL/SHM companions) before a clean v1 database is created. There is deliberately no stringly prototype translation. Model-facing schemas, book manifests, office messages, and typed reports are versioned independently at v1.
...
The planning graph is acyclic and versioned. A node becomes ready only when every prerequisite succeeded. Delegation is evidence-triggered: Economy stays single-lane, Balanced expands only when forecast speedup is at least 25% and coordination overhead at most 15%, and Turbo may use up to the validated 16-lane ceiling for truly disjoint work. The scheduler launches only ready tasks whose declared resources do not conflict.

The office layer provides typed records for agents, tasks, TODO deltas, evidence, artifacts, progress, anonymous health metrics, incidents, and replacement handoffs. TODO changes recompute a deterministic user-facing rollup of active goals, blocked work with reasons, and recently completed objectives; unchanged frames reuse those persisted deltas without a manager model call. Hive messages accept only finding, decision, blocker, request, progress, handoff, and artifact-reference deltas. Bodies are bounded, repeated normalized findings are deduplicated, and inbox reads return only unread recipient-relevant messages before advancing a durable consumption record. Board entries distinguish project-wide decisions/constraints from run-local findings and progress; project pins are user-controlled rather than a free model write.
...
Clarification is optimized separately. The local score makes clear prompts cost no intake model turn. Clarifier roles omit skill and compatible-agent discovery bodies and receive only instruction-file paths for targeted lookup, avoiding large repeated policy prefixes. Their state, original report, consultation, question count, turn count, input budget, and tool calls are independently capped. Confirmed briefs remove conversational ambiguity before the more expensive planning and worker stages.

Context uses a conservative, model-independent estimate rather than claiming tokenizer precision. Each agent tracks its own advertised and effective limits, forecast, output allowance, reserve, and capability source. Consumed tool outputs are replaced by typed digest-backed evidence summaries while raw events remain durable; independent read/search calls execute concurrently, while mutations remain serialized. Predictive compaction runs only when the next bounded call cannot fit below the effective 95 percent boundary (or after explicit `/compact`), uses the provider matching the configured compaction model, rejects empty summaries, retains complete recent call/result pairs, and records before/after evidence.

Memory is separate from compaction. Retrieval fuses FTS relevance with exact entity, scope, pin, confidence, and salience signals. User, project, and run scopes remain isolated; superseded and tombstoned records are hidden. Meaningful terminal runs queue bounded secret-filtered episodic extraction for later idle preflight processing. Injected memory is explicitly advisory below checked-in instructions and current evidence.
...
- SQLite schema v1 stores cache entries and statistics, clarification state, agent TODOs, memory indexes, provider-balance baselines, and office-room cursors. `/clean` removes expired entries and trims by least-recent use to the configured size.
...
The TUI `/status` command opens a real `status · inspector` dashboard. It refreshes local session/usage, per-agent context forecasts, cache, TODO rollup, office, account-profile, provider, memory, book, and problem data and can additionally append a compact status card to the transcript; the card is a secondary summary, not the inspector itself.
